...
public class TileMap_SquareEvenR : TileMap 
{
	...
	public override void CreatePaths()
	{
		...
		for (int i = 0; i < rows; i ++) 
		{
			for (int j = 0; j < columns; j ++) 
			{
				...
				//TOP-RIGHT
				__conections.Add (__sqr.positionOnGrid.y == 0 || (__sqr.positionOnGrid.x == columns - 1 && __sqr.positionOnGrid.y % 2 == 0) ? false : GetRandomConection());
				//BOTTOM-RIGHT
				__conections.Add (__sqr.positionOnGrid.y == rows - 1 || (__sqr.positionOnGrid.x == columns - 1 && __sqr.positionOnGrid.y % 2 == 0) ? false : GetRandomConection());
				//BOTTOM-LEFT
				__conections.Add (__sqr.positionOnGrid.y == rows - 1 || (__sqr.positionOnGrid.x == 0  && __sqr.positionOnGrid.y % 2 == 1) ? false : GetRandomConection());
				//TOP-LEFT
				__conections.Add (__sqr.positionOnGrid.y == 0 || (__sqr.positionOnGrid.x == 0 && __sqr.positionOnGrid.y % 2 == 1)? false : GetRandomConection());
	...
	{
		float xOffset = -0.7f - (Mathf.Clamp (columns - 3, 0, 100) * 0.4f);
