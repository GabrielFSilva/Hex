...
public class TileMap_SquareOddR : TileMap 
{
	...
	{
		base.CreateTileMap ();
		rows = Random.Range (4, 8);
		columns = Random.Range (rows -1 , rows);
		...
		for (int i = 0; i < rows; i ++) 
		{
			for (int j = 0; j < columns; j ++) 
			{
				...
				if (i % 2 == 0)
					__tempGO.transform.localPosition = new Vector3 (j * __2sqrt * 2f, -i  * __2sqrt, 0f);
				else
					__tempGO.transform.localPosition = new Vector3 ((j * __2sqrt * 2f) + __2sqrt, -i  * __2sqrt, 0f);
	...
	public override void CreatePaths()
	{
		...
		for (int i = 0; i < rows; i ++) 
		{
			for (int j = 0; j < columns; j ++) 
			{
				...
				//TOP-RIGHT
				__conections.Add (__sqr.positionOnGrid.y == 0 || (__sqr.positionOnGrid.x == columns - 1 && __sqr.positionOnGrid.y % 2 == 1) ? false : GetRandomConection());
				//BOTTOM-RIGHT
				__conections.Add (__sqr.positionOnGrid.y == rows - 1 || (__sqr.positionOnGrid.x == columns - 1 && __sqr.positionOnGrid.y % 2 == 1) ? false : GetRandomConection());
				//BOTTOM-LEFT
				__conections.Add (__sqr.positionOnGrid.y == rows - 1 || (__sqr.positionOnGrid.x == 0  && __sqr.positionOnGrid.y % 2 == 0) ? false : GetRandomConection());
				//TOP-LEFT
				__conections.Add (__sqr.positionOnGrid.y == 0 || (__sqr.positionOnGrid.x == 0 && __sqr.positionOnGrid.y % 2 == 0)? false : GetRandomConection());
	...
	{
		Vector3 __tempVec3 = new Vector3 (((tiles[1].transform.position.x + tiles[tiles.Count-2].transform.position.x)/2f) - 0.7f, 
