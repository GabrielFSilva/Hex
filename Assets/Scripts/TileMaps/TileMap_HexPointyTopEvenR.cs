...
public class TileMap_HexPointyTopEvenR : TileMap 
{
	...
	public override void CreatePaths()
	{
		...
		for (int i = 0; i < rows; i ++) 
		{
			for (int j = 0; j < columns; j ++) 
			{
				...
				//UP-RIGHT
				__conections.Add ((__hex.positionOnGrid.y == 0 || (__hex.positionOnGrid.x == columns - 1 && __hex.positionOnGrid.y % 2 == 0)) ? false : GetRandomConection());
				//RIGHT
				__conections.Add ((__hex.positionOnGrid.x == (columns - 1)) ? false : GetRandomConection());
				//BOTTOM-RIGHT
				__conections.Add ((__hex.positionOnGrid.y == (rows - 1) || (__hex.positionOnGrid.x == columns - 1 && __hex.positionOnGrid.y % 2 == 0)) ? false : GetRandomConection());
				//BOTTOM-LEFT
				__conections.Add ((__hex.positionOnGrid.y == (rows - 1) || (__hex.positionOnGrid.x == 0 && __hex.positionOnGrid.y % 2 == 1)) ? false : GetRandomConection());
				//LEFT
				__conections.Add ((__hex.positionOnGrid.x == 0) ? false : GetRandomConection());
				//UP-LEFT
				__conections.Add ((__hex.positionOnGrid.y == 0 || (__hex.positionOnGrid.x == 0 && __hex.positionOnGrid.y % 2 == 1)) ? false : GetRandomConection());
	...
	public override void CalcCameraPosition ()
	{
		...
		Vector3 __tempVec3 = new Vector3 (((tiles[columns].transform.position.x + tiles[tiles.Count-1 - columns].transform.position.x)/2f) + xOffset, 
		                                  (tiles[0].transform.position.y + tiles[tiles.Count-1].transform.position.y)/2f, -12f);
		if (rows % 2 == 1)
			__tempVec3 = new Vector3 (((tiles[columns].transform.position.x + tiles[tiles.Count-1].transform.position.x)/2f) + xOffset, 
			                          (tiles[0].transform.position.y + tiles[tiles.Count-1].transform.position.y)/2f, -12f);
