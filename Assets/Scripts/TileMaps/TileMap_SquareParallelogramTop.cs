...
public class TileMap_SquareParallelogramTop : TileMap 
{
	...
	public override void CreatePaths()
	{
		...
		for (int i = 0; i < rows; i ++) 
		{
			for (int j = 0; j < columns; j ++) 
			{
				...
				//TOP-RIGHT
				__conections.Add (__sqr.positionOnGrid.y == 0 || __sqr.positionOnGrid.x == columns - 1 ? false : GetRandomConection());
				//BOTTOM-RIGHT
				__conections.Add (__sqr.positionOnGrid.y == rows - 1 ? false : GetRandomConection());
				//BOTTOM-LEFT
				__conections.Add (__sqr.positionOnGrid.y == rows - 1 || __sqr.positionOnGrid.x == 0 ? false : GetRandomConection());
				//TOP-LEFT
